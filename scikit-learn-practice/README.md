...
| `03_preprocessing.py` | `StandardScaler` on California Housing (binary "above-median value" task): accuracy and convergence with/without scaling. | — |
...
while the scaled run converged cleanly. This is the honest, more useful
lesson: scaling's main effect here wasn't a couple points of accuracy, it was
whether the solver reliably reached its optimum at all. On features spanning
`Population` (3 to 35,682) vs `MedInc` (0.5 to 15), that's not surprising.
...
overfitting: the extra depth after 5 buys nothing on unseen data.
...
quantizing that same overfit model for a Pi does not fix it — it just ships
...
- A `ConvergenceWarning` is not just a nuisance to suppress — it means the
  reported coefficients/accuracy may not reflect the true optimum. It's worth
