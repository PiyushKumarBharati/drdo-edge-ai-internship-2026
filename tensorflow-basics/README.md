...
|---|---|---|
| `01_tensors.py` | `tf.constant`, `tf.Variable`, shapes, dtype, casting, NumPy interop. | (none) |
...
| `04_model_inspection.py` | `model.summary()`, per-layer parameter count, size estimation vs actual file size. | (none) |
...
(digits): 87% vs 97% test accuracy on comparably small models and epoch
budgets is the expected gap, not a bug.
...
float32 (4 bytes/param): 0.216 MB; at int8 (1 byte/param): 0.054 MB. The
actual `.keras` file on disk is 0.687 MB, larger than the raw weight
estimate because it also stores architecture and optimizer state (the
`.tflite` conversion in `tensorflow-lite/` strips that away).
...
signal that mattered. A `Dense` layer, by contrast, connects every input to
every output, which is exactly why the one `Dense(64)` layer after
`Flatten` (800 to 64 connections) costs more parameters (51,264) than both
...
`GlobalAveragePooling2D` (which has zero parameters) would be the
highest-leverage change, before ever touching quantization. Quantization
...
- Fashion-MNIST images are `(28, 28)` on load. A `Conv2D` layer needs a
  ...
  is required before it fits the model's `Input(shape=(28, 28, 1))`. Easy
